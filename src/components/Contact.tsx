...
import { Instagram, Calendar, ArrowUpRight } from "lucide-react";
        ...
        <section id="contact" className="py-24 sm:py-40 bg-black relative overflow-hidden">
            <div className="max-w-4xl mx-auto px-6 relative z-10 text-center space-y-20 lg:space-y-32">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-10">
                    ...
                    {/* Book a Demo — links to cal.com (replace URL when ready) */}
                    <motion.a
                        initial={{ opacity: 0, y: 20 }}
                        whileInView={{ opacity: 1, y: 0 }}
                        viewport={{ once: true }}
                        transition={{ delay: 0.1 }}
                        href="https://cal.com/YOUR_LINK"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex flex-col items-center justify-center p-10 sm:p-14 bg-white/[0.02] border border-white/5 hover:border-white/10 hover:bg-white/[0.03] transition-all group rounded-sm"
                        aria-label="Book a demo session"
                    >
                        <Calendar size={40} className="text-white/20 group-hover:text-white transition-colors mb-6" />
                        <div className="text-center space-y-2">
                            <p className="text-[10px] text-white/20 font-bold uppercase tracking-widest">Schedule</p>
                            <p className="text-base font-medium flex items-center justify-center">
                                Book a Demo <ArrowUpRight size={14} className="ml-1 opacity-20 group-hover:opacity-100 transition-opacity" />
                            </p>
                        </div>
                    </motion.a>
                </div>
